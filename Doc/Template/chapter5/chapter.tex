...
As shown in Figure \ref{fig:feature_clusters}, the best asset seems to involve the whole set of features, as it reaches these scores, with 70 estimators: \textit{Precision} = 0.946, \textit{Recall} = 0.945, \textbf{\textit{F1}}= 0.943.
\begin{figure}[htp!]
	\centering
...
We decided to stick with the following configuration without continuing with further explorations, because we could observe a flat improvements from 250 to 300 estimators, with that depth. Furthermore, we decided to have a light model, in order to keep the prediction time fast enough, which is currently measured in 33ms.
