...
The idea we had was to use the validation set to enrich the binary training set, in order to make the algorithm handle younger and different types of samples from the Twitter population.

We performed a 10-fold-crossvalidation to see the effect of this data refill, sticking to the same hyperparameters found by the last Grid Serch. The AUC score came back to an already met value: 0.951.

It could be imputed by the biased extrinsic features of that data: those samples came from the multiclass dataset and they originally had the extrinsic features based on the four bot categories' dictionaries.
...
Now, we wanted to take a look inside the inner ranking performed by the algorithm, to check if the age field represented an important splitting point.
Figure \ref{fig:bon_importances_ext} shows that the age attribute was dethroned from the top position. Indeed, it was replaced by the \textit{friends\_count} attribute.

The age feature was still determinant, as it was placed in the second spot of the features standing.
...

{
	...
	\begin{tabular}{@{}cccc@{}}
		...
		\multicolumn{1}{c|}{\textbf{0.951}} & 
		\multicolumn{1}{c|}{\textbf{0.932}} \\
		\cline{1-4}\\
	\end{tabular}\\
}

We could see a decreasing performance, as we were stepping away from the original fitted data and features. Even the plain extension from our collected data seemed to aggravate the evaluations. However, the validation earlier described reached a 0.721 value in the AUC score, with the test set coming from the multiclass dataset. Thus, we decided anyway to extend the training data with these samples, in order to refresh the decision rules of the forest and to make it see more heterogeneous data.

The age filed removing made thing worse, but we decided to perform it anyway, because of the good score reached without it, and the flexibility we were giving to the Random Forest. Without making it considering that field, we were removing a bias introduced by the older accounts of the Carvelee's dataset.
...
\end{figure}
The binary classifier has then been fitted with 37,212 samples with 33 features, 700 estimators, entropy splitting criterion and 26 levels of maximum depth.
